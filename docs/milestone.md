...
Done 2026-08-13:   PR #34 (legacy-trim -> master) opened · DIST_VERSION 1.2.1 -> 1.2.2 across
                     the three Dockerfiles (D22); upstream 1.2.1 removed, only 1.2.2 published
                     · #27 reopened for 1.2.2 re-verification (M2.5)
Done 2026-08-14:   PR #34 green on all three at 1.2.2 — build + gate 11/11 (M4.2 ✅, M5.2 ✅)
                   · M2.5 container-gate portion verified · stale 1.2.1 doc strings fixed
                     (README, gate.bash comment, D10) · con/procServ pinning deferred (D23/M7.5)

Next entry points:
  ▶ ready now:   PR #34 is green at 1.2.2 -> merge legacy-trim to master (Closes #31)
  planned order: merge (Closes #31) -> [G2] M5.3 Docker Hub publish (owner-run)
...
Operator action:  none standing — #31 rides PR #34 (`Closes #31`), closing only after the 1.2.2 CI-green merge, so docs are re-checked (M2.5) before it closes

Next session entry point: PR #34 is green at 1.2.2 on all three images (build +
container gate 11/11). Next is the master merge (its `Closes #31` closes the docs
issue), then the owner-run Docker Hub publish (M5.3 <- G2). M2.5 stays open only
for the deep-gate re-run (external alliocs harness, D17) and image-size recapture.
```
...
| M2 | Distribution-based images (#27) | 4/5 | 🔄 | (M2.5 re-verify at 1.2.2, pending PR #34) |
...
| M7 | Deferred follow-ups (#32) | 0/5 | ⬜ | (M7.5 Conditional on D23) |
...
| | M4.2 | Wire the gates into CI per image build | ✅ | | ← M4.1 · ← M5.2 | 2026-07-19: implemented inside the reusable `image.yml` (M5.2) — build+`load` then `docker run` the loaded image against `gate.bash`; publish only runs after the gate step. Verified 2026-08-14: PR #34 (commit 36d49ad) ran the Verification gate step on all three images at 1.2.2 and passed — debian13 G0-G10 11/11 (`/opt/epics/1.2.2/debian-13/7.0.10`), rocky8/rocky10 gate step success (G1 64/64, G10 manifest present). |
...
| | M7.5 | Pin con and procServ-env to a released version (currently floating default-branch HEAD) | Conditional | | ← D23 | Condition: procServ-env publishes a version number. When observed, pin both clones to a tag or commit in all three Dockerfiles and record it in the bake manifest. |
...
| D10 | Tag scheme: image names stay `<os>-epics` (GitLab consumer contract); version tags follow the `EPICS-env-distribution` release (e.g. `debian13-epics:1.2.2`); `latest` tracks the newest; EPICS base version recorded in image labels (session D-3) | 2026-07-18 |
...
- The M3 task IDs intentionally skip M3.2: the original M3.2 (`tools` IOC
  generator) was moved to the backlog as M7.4 (D19). Existing dependency arrows
  keep their IDs rather than renumber.
